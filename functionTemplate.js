exports.funk = `
	...
	function todoing (cont) {
		...
		tx = tx.replace(/([\\s\\S]*?)\\<(todolist) value="([\\s\\S]*?)"\\>([\\s\\S]*?)\\<\\/(todolist)\\>/gi,'(function todoing$2 ()  {var conty = document.getElementById("'+cont+'"); var todoin = document.createElement("input"); var deb = \`$1\`; conty.innerHTML+= deb; conty.appendChild(todoin); todoin.id = "todoin";var bout = document.createElement("button");bout.id = "boutTodo"; bout.innerHTML = "$3"; conty.appendChild(bout); var ul = document.createElement("ul");conty.appendChild(ul);  bout.addEventListener('click', function () {var tx = todoin.value;	var li = document.createElement("li");	li.innerHTML = tx;	var del = document.createElement("span");	del.innerHTML = "[x]";	del.addEventListener("click",function () { remove(li); },false);	li.appendChild(del);	ul.appendChild(li); todoin.value = ""; });\n\n function remove (el) { ul.removeChild(el);  }})()');
		tx = tx.replace(/(\\<temp\\>)/gi,'');
		tx = tx.replace(/(\\<\\/temp\\>)/gi,'');
		tx = tx.replace(/\\$key.([\\s\\S]*?)\\$/gi,'\`+dat[key].$1+\`');
		var sc = document.createElement("script");
